...
float pixelToLat(float pixel_y) {
    ...
    return (2.0f * atanf(expf((pixel_y - 3988.0f) / -1296.0f)) - (float)M_PI / 2.0f) * 180.0f / (float)M_PI;
}
...
void formatCoord(char* buf, size_t size, float lat, float lon) {
    ...
    snprintf(buf, size, "%.2f° %c %.2f° %c", fabsf(lat), latDir, fabsf(lon), lonDir);
}
...
void drawRoundedRect(SDL_Renderer* ren, SDL_Rect rect, int radius, SDL_Color color, int glossy) {
    ...
    for (int w = 0; w < radius * 2; w++) {
        for (int h = 0; h < radius * 2; h++) {
            ...
            if (dx*dx + dy*dy <= radius*radius) {
                ...
                SDL_RenderDrawPoint(ren, rect.x + rect.w - w - 1, rect.y + h); // Top-right
                SDL_RenderDrawPoint(ren, rect.x + w, rect.y + rect.h - h - 1); // Bottom-left
                SDL_RenderDrawPoint(ren, rect.x + rect.w - w - 1, rect.y + rect.h - h - 1); // Bottom-right
            }
        }
    }
    ...
    if (glossy) {
        ...
        for (int i = 0; i < glossHeight; i++) {
            ...
            if (i < radius) {
                int cornerOffset = (int)sqrtf(radius*radius - (radius-i)*(radius-i));
...
void drawRoundedRectBorder(SDL_Renderer* ren, SDL_Rect rect, int radius, SDL_Color color) {
    SDL_SetRenderDrawColor(ren, color.r, color.g, color.b, color.a);
    
    // Draw straight edges
    SDL_RenderDrawLine(ren, rect.x + radius, rect.y, rect.x + rect.w - radius - 1, rect.y); // Top
    SDL_RenderDrawLine(ren, rect.x + radius, rect.y + rect.h - 1, rect.x + rect.w - radius - 1, rect.y + rect.h - 1); // Bottom
    SDL_RenderDrawLine(ren, rect.x, rect.y + radius, rect.x, rect.y + rect.h - radius - 1); // Left
    SDL_RenderDrawLine(ren, rect.x + rect.w - 1, rect.y + radius, rect.x + rect.w - 1, rect.y + rect.h - radius - 1); // Right
    ...
    for (int w = 0; w < radius * 2; w++) {
        for (int h = 0; h < radius * 2; h++) {
            ...
            if (dist <= radius*radius && dist > (radius-1)*(radius-1)) {
                ...
                if (w >= radius && h <= radius) {
                    SDL_RenderDrawPoint(ren, rect.x + rect.w - (radius*2 - w), rect.y + h);
                }
                // Bottom-left corner (only draw if in bottom-left quadrant)
                if (w <= radius && h >= radius) {
                    SDL_RenderDrawPoint(ren, rect.x + w, rect.y + rect.h - (radius*2 - h));
                }
                // Bottom-right corner (only draw if in bottom-right quadrant)
                if (w >= radius && h >= radius) {
                    SDL_RenderDrawPoint(ren, rect.x + rect.w - (radius*2 - w), rect.y + rect.h - (radius*2 - h));
                }
            }
        }
    }
...
void calculateRouteStats() {
    ...
    // Estimate fuel consumption (rough estimate: ~0.5 tons per 100km for cargo ship)
    routeFuel = routeDistance * 0.5f / 100.0f;
    
    // Risk percentage
    routeRisk = maxRisk;
    if (routeRisk > 100.0f) routeRisk = 100.0f;
...
int main(int argc, char* argv[]) {
    ...
    TTF_Font* valueFont = TTF_OpenFont("assets/fonts/JetBrainsMono-ExtraBold.ttf", 18);  // For numbers/values
    TTF_Font* labelFont = TTF_OpenFont("assets/fonts/Inter_18pt-Regular.ttf", 14);       // For labels
    TTF_Font* titleFont = TTF_OpenFont("assets/fonts/Inter_18pt-Regular.ttf", 16);       // For title
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_MOUSEWHEEL) {
                ...
                // Store the world position under the mouse and the mouse screen position
                ...
                if (targetZoom < 0.289f) targetZoom = 0.289f;
                if (targetZoom > 2.0f) targetZoom = 2.0f;  // Limit zoom out
        ...
        // Smooth zoom interpolation with camera adjustment to keep world point under mouse
        float prevZoom = zoom;
        ...
        if (fabs(targetZoom - zoom) > 0.001f) {
            camX = zoomWorldX - (zoomMouseX - WIDTH / 2.0f) / zoom;
            camY = zoomWorldY - (zoomMouseY - HEIGHT / 2.0f - TOPBAR) / zoom;
        ...
        if (p1.valid || p2.valid) {
            ...
            snprintf(coords, sizeof(coords), "A: (%d, %d) B: (%d, %d) | %s", 
                     px1, py1, px2, py2, infoText);
        ...
        SDL_Surface* titleSurf = TTF_RenderText_Blended(titleFont, "ROUTE ANALYSIS", (SDL_Color){0, 255, 255, 255});
        ...
        // Status
        SDL_Surface* statusLabel = TTF_RenderText_Blended(labelFont, "Status", (SDL_Color){150, 150, 150, 255});
        SDL_Texture* statusLabelTex = SDL_CreateTextureFromSurface(ren, statusLabel);
        SDL_Rect statusLabelRect = {panel.x + 15, yPos, statusLabel->w, statusLabel->h};
        SDL_RenderCopy(ren, statusLabelTex, NULL, &statusLabelRect);
        SDL_FreeSurface(statusLabel); SDL_DestroyTexture(statusLabelTex);
        
        SDL_Color statusColor = strcmp(routeStatus, "OPTIMIZED") == 0 ? 
            (SDL_Color){0, 255, 100, 255} : (SDL_Color){255, 200, 0, 255};
        SDL_Surface* statusVal = TTF_RenderText_Blended(valueFont, routeStatus, statusColor);
        SDL_Texture* statusValTex = SDL_CreateTextureFromSurface(ren, statusVal);
        SDL_Rect statusValRect = {panel.x + panel.w - 15 - statusVal->w, yPos - 2, statusVal->w, statusVal->h};
        SDL_RenderCopy(ren, statusValTex, NULL, &statusValRect);
        SDL_FreeSurface(statusVal); SDL_DestroyTexture(statusValTex);
        yPos += 35;
        
        // Total Distance
        SDL_Surface* distLabel = TTF_RenderText_Blended(labelFont, "Total Distance", (SDL_Color){150, 150, 150, 255});
        ...
        SDL_FreeSurface(distLabel); SDL_DestroyTexture(distLabelTex);
        
        char distStr[64];
        snprintf(distStr, sizeof(distStr), "%.0f", routeDistance);
        SDL_Surface* distVal = TTF_RenderText_Blended(valueFont, distStr, (SDL_Color){255, 255, 255, 255});
        SDL_Texture* distValTex = SDL_CreateTextureFromSurface(ren, distVal);
        SDL_Rect distValRect = {panel.x + panel.w - 70 - distVal->w, yPos - 2, distVal->w, distVal->h};
        SDL_RenderCopy(ren, distValTex, NULL, &distValRect);
        SDL_FreeSurface(distVal); SDL_DestroyTexture(distValTex);
        
        SDL_Surface* kmLabel = TTF_RenderText_Blended(labelFont, "km", (SDL_Color){100, 100, 100, 255});
        SDL_Texture* kmLabelTex = SDL_CreateTextureFromSurface(ren, kmLabel);
        SDL_Rect kmLabelRect = {panel.x + panel.w - 40, yPos, kmLabel->w, kmLabel->h};
        SDL_RenderCopy(ren, kmLabelTex, NULL, &kmLabelRect);
        SDL_FreeSurface(kmLabel); SDL_DestroyTexture(kmLabelTex);
        yPos += 35;
        
        // ETA
        SDL_Surface* etaLabel = TTF_RenderText_Blended(labelFont, "ETA", (SDL_Color){150, 150, 150, 255});
        ...
        SDL_FreeSurface(etaLabel); SDL_DestroyTexture(etaLabelTex);
        
        char etaStr[64];
        snprintf(etaStr, sizeof(etaStr), "%.1f", routeETA);
        SDL_Surface* etaVal = TTF_RenderText_Blended(valueFont, etaStr, (SDL_Color){0, 255, 255, 255});
        SDL_Texture* etaValTex = SDL_CreateTextureFromSurface(ren, etaVal);
        SDL_Rect etaValRect = {panel.x + panel.w - 80 - etaVal->w, yPos - 2, etaVal->w, etaVal->h};
        SDL_RenderCopy(ren, etaValTex, NULL, &etaValRect);
        SDL_FreeSurface(etaVal); SDL_DestroyTexture(etaValTex);
        
        SDL_Surface* daysLabel = TTF_RenderText_Blended(labelFont, "days", (SDL_Color){100, 100, 100, 255});
        SDL_Texture* daysLabelTex = SDL_CreateTextureFromSurface(ren, daysLabel);
        SDL_Rect daysLabelRect = {panel.x + panel.w - 55, yPos, daysLabel->w, daysLabel->h};
        SDL_RenderCopy(ren, daysLabelTex, NULL, &daysLabelRect);
        SDL_FreeSurface(daysLabel); SDL_DestroyTexture(daysLabelTex);
        yPos += 40;
        ...
        // Fuel Estimate
        SDL_Surface* fuelLabel = TTF_RenderText_Blended(labelFont, "Fuel Estimate", (SDL_Color){0, 180, 200, 255});
        ...
        SDL_FreeSurface(fuelLabel); SDL_DestroyTexture(fuelLabelTex);
        
        char fuelStr[64];
        snprintf(fuelStr, sizeof(fuelStr), "%.1f tons", routeFuel);
        SDL_Surface* fuelVal = TTF_RenderText_Blended(valueFont, fuelStr, (SDL_Color){200, 200, 200, 255});
        SDL_Texture* fuelValTex = SDL_CreateTextureFromSurface(ren, fuelVal);
        SDL_Rect fuelValRect = {panel.x + panel.w - 15 - fuelVal->w, yPos, fuelVal->w, fuelVal->h};
        SDL_RenderCopy(ren, fuelValTex, NULL, &fuelValRect);
        SDL_FreeSurface(fuelVal); SDL_DestroyTexture(fuelValTex);
        yPos += 25;
        
        // Max Risk
        SDL_Surface* riskLabel = TTF_RenderText_Blended(labelFont, "Max Risk", (SDL_Color){0, 180, 200, 255});
        ...
        SDL_FreeSurface(riskLabel); SDL_DestroyTexture(riskLabelTex);
        
        char riskStr[64];
        snprintf(riskStr, sizeof(riskStr), "%.0f%%", routeRisk);
        SDL_Surface* riskVal = TTF_RenderText_Blended(valueFont, riskStr, (SDL_Color){0, 255, 100, 255});
        SDL_Texture* riskValTex = SDL_CreateTextureFromSurface(ren, riskVal);
        SDL_Rect riskValRect = {panel.x + panel.w - 15 - riskVal->w, yPos, riskVal->w, riskVal->h};
        SDL_RenderCopy(ren, riskValTex, NULL, &riskValRect);
        SDL_FreeSurface(riskVal); SDL_DestroyTexture(riskValTex);
        yPos += 30;
        
        // Optimization note
        if (strcmp(routeStatus, "OPTIMIZED") == 0) {
            char noteStr[128];
            snprintf(noteStr, sizeof(noteStr), "*V5 Optimized: %.0fkm*", routeDistance);
            SDL_Surface* note = TTF_RenderText_Blended(labelFont, noteStr, (SDL_Color){100, 120, 140, 255});
            ...
            SDL_FreeSurface(note); SDL_DestroyTexture(noteTex);
        }

        // --- Mouse Coordinate Panel (Bottom Right) ---
        // --- Coordinate Panel (A / B) ---
int panelX = currW - 290;   // right aligned
int panelY = 900;          // 1000px from top
...
SDL_Surface* aSurf = TTF_RenderText_Blended(valueFont, coordA, (SDL_Color){0, 255, 255, 255});
SDL_Surface* bSurf = TTF_RenderText_Blended(valueFont, coordB, (SDL_Color){0, 255, 255, 255});
...
SDL_Rect vesselPanel = { vesselPanelX, vesselPanelY, vesselPanelW, vesselPanelH };
...
// Header row
SDL_Surface* hVessel = TTF_RenderText_Blended(labelFont, "VESSEL", (SDL_Color){120,180,200,255});
SDL_Surface* hSpeed  = TTF_RenderText_Blended(labelFont, "SPEED",  (SDL_Color){120,180,200,255});
SDL_Surface* hMode   = TTF_RenderText_Blended(labelFont, "MODE",   (SDL_Color){120,180,200,255});
...
SDL_RenderCopy(ren, hVesselT, NULL, &(SDL_Rect){colVessel, vesselPanel.y + 10, hVessel->w, hVessel->h});
SDL_RenderCopy(ren, hSpeedT,  NULL, &(SDL_Rect){colSpeed,  vesselPanel.y + 10, hSpeed->w,  hSpeed->h});
SDL_RenderCopy(ren, hModeT,   NULL, &(SDL_Rect){colMode,   vesselPanel.y + 10, hMode->w,   hMode->h});

SDL_FreeSurface(hVessel); SDL_FreeSurface(hSpeed); SDL_FreeSurface(hMode);
SDL_DestroyTexture(hVesselT); SDL_DestroyTexture(hSpeedT); SDL_DestroyTexture(hModeT);
...
SDL_RenderDrawLine(
    ...
    vesselPanel.x + 15,
    vesselPanel.y + 38,
    vesselPanel.x + vesselPanel.w - 15,
    vesselPanel.y + 38
);

// Value row
SDL_Surface* vName  = TTF_RenderText_Blended(smallFont, shipName,  (SDL_Color){255,255,255,255});
SDL_Surface* vSpeed = TTF_RenderText_Blended(smallFont, shipSpeed, (SDL_Color){0,255,255,255});
SDL_Surface* vMode  = TTF_RenderText_Blended(smallFont, shipMode,  (SDL_Color){0,255,100,255});
...
SDL_RenderCopy(ren, vNameT,  NULL, &(SDL_Rect){colVessel, vesselPanel.y + 45, vName->w,  vName->h});
SDL_RenderCopy(ren, vSpeedT, NULL, &(SDL_Rect){colSpeed,  vesselPanel.y + 45, vSpeed->w, vSpeed->h});
SDL_RenderCopy(ren, vModeT,  NULL, &(SDL_Rect){colMode,   vesselPanel.y + 45, vMode->w,  vMode->h});

SDL_FreeSurface(vName); SDL_FreeSurface(vSpeed); SDL_FreeSurface(vMode);
SDL_DestroyTexture(vNameT); SDL_DestroyTexture(vSpeedT); SDL_DestroyTexture(vModeT);
